...
uint8_t u8g2_esp32_spi_byte_cb(u8x8_t* u8x8,
  ...
  switch (msg) {
    ...
    case U8X8_MSG_BYTE_INIT: {
      ...
      dev_config.clock_speed_hz = 15000000UL;
...
uint8_t u8g2_esp32_gpio_and_delay_cb(u8x8_t* u8x8,
  ...
  switch (msg) {
    ...
    case U8X8_MSG_GPIO_AND_DELAY_INIT: {
      uint64_t bitmask_input = 0;
      uint64_t bitmask_output = 0;
      gpio_config_t gpioConfig;
      if (u8g2_esp32_hal.dc != U8G2_ESP32_HAL_UNDEFINED) {
        bitmask_output = bitmask_output | (1ull << u8g2_esp32_hal.dc);
      }
      if (u8g2_esp32_hal.reset != U8G2_ESP32_HAL_UNDEFINED) {
        bitmask_output = bitmask_output | (1ull << u8g2_esp32_hal.reset);
      }
      if (u8g2_esp32_hal.bus.spi.cs != U8G2_ESP32_HAL_UNDEFINED) {
        bitmask_output = bitmask_output | (1ull << u8g2_esp32_hal.bus.spi.cs);
      }

      for (uint8_t i = 0; i < U8X8_PIN_CNT; i++) {
        if (u8x8->pins[i] != U8X8_PIN_NONE) {
          if (i < U8X8_PIN_OUTPUT_CNT) {
            bitmask_output = bitmask_output | (1ull << u8x8->pins[i]);
          } else {
            bitmask_input = bitmask_input | (1ull << u8x8->pins[i]);
          }
        }
      }

      if (bitmask_output != 0) {
        gpioConfig.pin_bit_mask = bitmask_output;
      ...
      ESP_LOGI(TAG, "bitmask_input = %llu", bitmask_input);

      if (bitmask_input != 0) {
        gpioConfig.pin_bit_mask = bitmask_input;
      ...
      if (msg >= U8X8_MSG_GPIO(0)) {
        ...
        if (i != U8X8_PIN_NONE) {
          if (u8x8_GetPinIndex(u8x8, msg) < U8X8_PIN_OUTPUT_CNT) {
            ...
            if (u8x8_GetPinIndex(u8x8, msg) == U8X8_PIN_OUTPUT_CNT) {
              // call yield() for the first pin only, u8x8 will always request
              // all the pins, so this should be ok
              taskYIELD();
            }
            u8x8_SetGPIOResult(u8x8, gpio_get_level(i) == 0 ? 0 : 1);
